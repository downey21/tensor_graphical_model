# this code obtains the "Sequential" estimators for Model 7 in Table 1 in the supplementary material.
rm(list = ls())
library(Tlasso)
library(tensr)
library(expm)
...
for (run in 1:Run) {
  ...
  for (c in 1:C.length) {
    ...
    fit <- Tlasso.fit(x, T = 1, lambda.vec = lambda) # T=1: one iteration
  ...
  C.best <- C[which.max(loglik)] # optimal value of C

  ## Model fitting
  fit <- Tlasso.fit(x, T = 1, lambda.vec = C.best * lam1)
